...
const styles = StyleSheet.create({
  ...
  const handleSectionPress = (section) => {
    if (selectedSection === section) {
      // Double-click detected, reset to default filter
      setSelectedSection('all');
    } else {
      // Set the selected section and scroll to top
      setSelectedSection(section);
      setTimeout(() => {
        flatListRef.current?.scrollToOffset({ offset: 0, animated: true });
      }, 0);
   ...
   {getDataForSection().length === 0 ? (
  ...
  ) : getDataForSection().length === 1 ? (
    // New UI for 1 Video
    <View style={styles.singleVideoContainer}>
      {renderStoryItem({ item: getDataForSection()[0], index: 0 })}
    </View>
  ) : getDataForSection().length === 2 ? (
    // New UI for 2 Videos
    <View style={styles.twoVideoContainer}>
      {getDataForSection().map((item, index) => renderStoryItem({ item, index }))}
    </View>
  ) : (
    // Old UI for More than 2 Videos
    <FlatList
      ref={flatListRef}
      data={getDataForSection()}
      renderItem={renderStoryItem}
    ...
    // twoVideoContainer: {
      // flexDirection: 'coloum',    // Horizontal layout
      // flexGrow: 1,

      // // justifyContent: 'space-between', // Space between the two videos
      // alignItems: 'center',    // Center the videos vertically
